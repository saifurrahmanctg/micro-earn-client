...
const MySubmissions = () => {
    ...
    return (
        <div className="fade-in">
            <div className="mb-10">
                <h2 className="text-3xl font-bold text-[#333333]">My <span className="text-[#2bb673]">Submissions</span></h2>
                <p className="text-gray-500 font-medium">Track the status of your completed tasks</p>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-premium overflow-hidden">
                                ...
                                <tr key={sub._id} className="hover:bg-green-50/20 transition-colors">
                                    <td className="py-5 px-8">
                                        <div className="flex flex-col">
                                            <span className="font-bold text-[#333333] mb-1">{sub.task_title}</span>
                                            <span className="text-xs text-gray-400 font-medium line-clamp-1">Proof: {sub.submission_details}</span>
                                        </div>
                                    </td>
                                    <td>
                                        <div className="text-sm font-bold text-gray-500">{sub.buyer_name}</div>
                                    </td>
                ...
                {/* Pagination */}
                {totalPages > 1 && (
                    <div className="p-6 bg-[#f9f9f9] border-t border-gray-50 flex justify-center items-center gap-4">
                        <button 
                            disabled={page === 0}
                            onClick={() => setPage(p => p - 1)}
                            className={`w-10 h-10 flex items-center justify-center rounded-lg border transition-all ${page === 0 ? 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed' : 'bg-white text-[#2bb673] border-gray-200 hover:bg-[#2bb673] hover:text-white shadow-sm'}`}
                        >
                            <FaChevronLeft />
                        </button>
                        ...
                        <div className="flex gap-2">
                            {[...Array(totalPages)].map((_, i) => (
                                <button
                                    key={i}
                                    onClick={() => setPage(i)}
                                    className={`w-10 h-10 rounded-lg font-bold text-sm transition-all ${page === i ? 'bg-[#2bb673] text-white shadow-lg' : 'bg-white text-gray-500 border border-gray-200 hover:border-[#2bb673] hover:text-[#2bb673]'}`}
                                >
                                    {i + 1}
                                </button>
                            ))}
                        ...
                        <button 
                            ...
                            onClick={() => setPage(p => p + 1)}
                            className={`w-10 h-10 flex items-center justify-center rounded-lg border transition-all ${page === totalPages - 1 ? 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed' : 'bg-white text-[#2bb673] border-gray-200 hover:bg-[#2bb673] hover:text-white shadow-sm'}`}
                        >
                            <FaChevronRight />
                        </button>
                    </div>
